...
LCMToROSRepublisher::LCMToROSRepublisher(ros::NodeHandle& n) 
{
    ...
    // Publisher/s
    ros_pub = nh.advertise<ark_bridge::Path>("strategy_manager_simple_path", 10);
    ROS_DEBUG("strategy_manager_simple_path publisher created");
};

LCMToROSRepublisher::~LCMToROSRepublisher() 
{    
    ROS_INFO("strategy_manager_simple_path LCMToROSRepublisher destructor.");
}
        ...
        const ark::Path* msg)
{
    ...
    const ark_bridge::Path * new_ros_msg = reinterpret_cast<const ark_bridge::Path *>(msg);
...
int main(int argc, char** argv)
{
    ...
    ros::init(argc, argv, "strategy_manager_simple_path_republish");
    ros::NodeHandle nh;   

    LCMToROSRepublisher handlerObject(nh);
    lcm.subscribe("strategy_manager_simple_path", &LCMToROSRepublisher::lcmCallback, &handlerObject);
