...
LCMToROSRepublisher::LCMToROSRepublisher(ros::NodeHandle& n) 
{
    ...
    // Publisher/s
    ros_pub = nh.advertise<ark_bridge::Empty>("control_selection_autonomy_disable_response", 10);
    ROS_DEBUG("control_selection_autonomy_disable_response publisher created");
};

LCMToROSRepublisher::~LCMToROSRepublisher() 
{    
    ROS_INFO("control_selection_autonomy_disable_response LCMToROSRepublisher destructor.");
}
...
int main(int argc, char** argv)
{
    ...
    ros::init(argc, argv, "control_selection_autonomy_disable_response_republish");
    ros::NodeHandle nh;   

    LCMToROSRepublisher handlerObject(nh);
    lcm.subscribe("control_selection_autonomy_disable_response", &LCMToROSRepublisher::lcmCallback, &handlerObject);
