...
LCMToROSRepublisher::LCMToROSRepublisher(ros::NodeHandle& n) 
{
    ...
    // Publisher/s
    ros_pub = nh.advertise<ark_bridge::Float64>("slam_localization_score", 10);
    ROS_DEBUG("slam_localization_score publisher created");
};

LCMToROSRepublisher::~LCMToROSRepublisher() 
{    
    ROS_INFO("slam_localization_score LCMToROSRepublisher destructor.");
}
        ...
        const ark::Float64* msg)
{
    ...
    const ark_bridge::Float64 * new_ros_msg = reinterpret_cast<const ark_bridge::Float64 *>(msg);
...
int main(int argc, char** argv)
{
    ...
    ros::init(argc, argv, "slam_localization_score_republish");
    ros::NodeHandle nh;   

    LCMToROSRepublisher handlerObject(nh);
    lcm.subscribe("slam_localization_score", &LCMToROSRepublisher::lcmCallback, &handlerObject);
