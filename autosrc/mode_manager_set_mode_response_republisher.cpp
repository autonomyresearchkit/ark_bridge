...
LCMToROSRepublisher::LCMToROSRepublisher(ros::NodeHandle& n) 
{
    ...
    // Publisher/s
    ros_pub = nh.advertise<ark_bridge::Status>("mode_manager_set_mode_response", 10);
    ROS_DEBUG("mode_manager_set_mode_response publisher created");
};

LCMToROSRepublisher::~LCMToROSRepublisher() 
{    
    ROS_INFO("mode_manager_set_mode_response LCMToROSRepublisher destructor.");
}
        ...
        const ark::Status* msg)
{
    ...
    const ark_bridge::Status * new_ros_msg = reinterpret_cast<const ark_bridge::Status *>(msg);
...
int main(int argc, char** argv)
{
    ...
    ros::init(argc, argv, "mode_manager_set_mode_response_republish");
    ros::NodeHandle nh;   

    LCMToROSRepublisher handlerObject(nh);
    lcm.subscribe("mode_manager_set_mode_response", &LCMToROSRepublisher::lcmCallback, &handlerObject);
