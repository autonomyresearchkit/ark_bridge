...
LCMToROSRepublisher::LCMToROSRepublisher(ros::NodeHandle& n) 
{
    ...
    // Publisher/s
    ros_pub = nh.advertise<ark_bridge::Bool>("strategy_manager_path_complete", 10);
    ROS_DEBUG("strategy_manager_path_complete publisher created");
};

LCMToROSRepublisher::~LCMToROSRepublisher() 
{    
    ROS_INFO("strategy_manager_path_complete LCMToROSRepublisher destructor.");
}
...
int main(int argc, char** argv)
{
    ...
    ros::init(argc, argv, "strategy_manager_path_complete_republish");
    ros::NodeHandle nh;   

    LCMToROSRepublisher handlerObject(nh);
    lcm.subscribe("strategy_manager_path_complete", &LCMToROSRepublisher::lcmCallback, &handlerObject);
