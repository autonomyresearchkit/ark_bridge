...
LCMToROSRepublisher::LCMToROSRepublisher(ros::NodeHandle& n) 
{
    ...
    // Publisher/s
    ros_pub = nh.advertise<ark_bridge::Bool>("slam_set_initial_pose_response", 10);
    ROS_DEBUG("slam_set_initial_pose_response publisher created");
};

LCMToROSRepublisher::~LCMToROSRepublisher() 
{    
    ROS_INFO("slam_set_initial_pose_response LCMToROSRepublisher destructor.");
}
...
int main(int argc, char** argv)
{
    ...
    ros::init(argc, argv, "slam_set_initial_pose_response_republish");
    ros::NodeHandle nh;   

    LCMToROSRepublisher handlerObject(nh);
    lcm.subscribe("slam_set_initial_pose_response", &LCMToROSRepublisher::lcmCallback, &handlerObject);
