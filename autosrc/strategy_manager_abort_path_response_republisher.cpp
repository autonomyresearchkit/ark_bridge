...
LCMToROSRepublisher::LCMToROSRepublisher(ros::NodeHandle& n) 
{
    ...
    // Publisher/s
    ros_pub = nh.advertise<ark_bridge::Empty>("strategy_manager_abort_path_response", 10);
    ROS_DEBUG("strategy_manager_abort_path_response publisher created");
};

LCMToROSRepublisher::~LCMToROSRepublisher() 
{    
    ROS_INFO("strategy_manager_abort_path_response LCMToROSRepublisher destructor.");
}
...
int main(int argc, char** argv)
{
    ...
    ros::init(argc, argv, "strategy_manager_abort_path_response_republish");
    ros::NodeHandle nh;   

    LCMToROSRepublisher handlerObject(nh);
    lcm.subscribe("strategy_manager_abort_path_response", &LCMToROSRepublisher::lcmCallback, &handlerObject);
