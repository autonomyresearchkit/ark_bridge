...
LCMToROSRepublisher::LCMToROSRepublisher(ros::NodeHandle& n) 
{
    ...
    // Publisher/s
    ros_pub = nh.advertise<ark_bridge::Empty>("map_data_save_map_to_disk_response", 10);
    ROS_DEBUG("map_data_save_map_to_disk_response publisher created");
};

LCMToROSRepublisher::~LCMToROSRepublisher() 
{    
    ROS_INFO("map_data_save_map_to_disk_response LCMToROSRepublisher destructor.");
}
...
int main(int argc, char** argv)
{
    ...
    ros::init(argc, argv, "map_data_save_map_to_disk_response_republish");
    ros::NodeHandle nh;   

    LCMToROSRepublisher handlerObject(nh);
    lcm.subscribe("map_data_save_map_to_disk_response", &LCMToROSRepublisher::lcmCallback, &handlerObject);
