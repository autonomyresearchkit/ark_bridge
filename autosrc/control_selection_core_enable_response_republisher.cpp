...
LCMToROSRepublisher::LCMToROSRepublisher(ros::NodeHandle& n) 
{
    ...
    // Publisher/s
    ros_pub = nh.advertise<ark_bridge::Empty>("control_selection_core_enable_response", 10);
    ROS_DEBUG("control_selection_core_enable_response publisher created");
};

LCMToROSRepublisher::~LCMToROSRepublisher() 
{    
    ROS_INFO("control_selection_core_enable_response LCMToROSRepublisher destructor.");
}
...
int main(int argc, char** argv)
{
    ...
    ros::init(argc, argv, "control_selection_core_enable_response_republish");
    ros::NodeHandle nh;   

    LCMToROSRepublisher handlerObject(nh);
    lcm.subscribe("control_selection_core_enable_response", &LCMToROSRepublisher::lcmCallback, &handlerObject);
