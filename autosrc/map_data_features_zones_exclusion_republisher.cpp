...
LCMToROSRepublisher::LCMToROSRepublisher(ros::NodeHandle& n) 
{
    ...
    // Publisher/s
    ros_pub = nh.advertise<ark_bridge::GeometryArray>("map_data_features_zones_exclusion", 10);
    ROS_DEBUG("map_data_features_zones_exclusion publisher created");
};

LCMToROSRepublisher::~LCMToROSRepublisher() 
{    
    ROS_INFO("map_data_features_zones_exclusion LCMToROSRepublisher destructor.");
}
...
int main(int argc, char** argv)
{
    ...
    ros::init(argc, argv, "map_data_features_zones_exclusion_republish");
    ros::NodeHandle nh;   

    LCMToROSRepublisher handlerObject(nh);
    lcm.subscribe("map_data_features_zones_exclusion", &LCMToROSRepublisher::lcmCallback, &handlerObject);
