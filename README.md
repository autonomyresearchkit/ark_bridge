...
```bash
cd ~/catkin_ws/src
git clone git@gitlab.clearpathrobotics.com:ark/ark_bridge.git
cd ark_bridge
```
...
4. Confirm that it worked!  
    1. In a new terminal, run the launch file: `roslaunch ark_bridge example_republishers.launch`
    2. In a new terminal:
        - Run `rostopic list` and confirm that the topics */ark_bridge/example_topic* and */ark_bridge/other_topic* exist.  
        - Then, show incoming messages using: `rostopic echo /ark_bridge/example_topic`
    3. Finally, in a third terminal: `rosrun ark_bridge example_send_lcm`. This is a small LCM-only (no ROS dependencies) program that publishes a single message onto the LCM topic *example_topic*
...
```bash
roslaunch ark_bridge CONFIG_NAME.launch
```
(where `CONFIG_NAME` is the name of the config file without file extension). Note that this launch file places all republishers under the namespace `\ark_bridge`.  
Each republisher can also be launched separately with: `rosrun ark_bridge TOPIC_NAME_republisher`. As usual, output topics can be remapped using standard ROS commands if required.
